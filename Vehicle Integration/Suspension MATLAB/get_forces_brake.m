function [F, Mz, SA, solved] = get_forces_brake(v, accel, delta, param)
% GET_FORCES_BRAKE Get forces on all tires required to cause given
% longitudinal and lateral acceleration under braking
%   Inputs:
%       v: car's velocity
...
%     b = abs(const.rr.tcp(1) - const.cg(1));
    ...
    f_brake = @(x) ...
        ...
        [x_brake, ~, flag, ~] = fsolve(f_brake, x_init, options);

        solved = flag >= 1;

        F = reshape(x_brake(1:12), 3, 4);
